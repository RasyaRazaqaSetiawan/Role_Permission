                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('role.store') }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input id="roleName" value="{{ old('name', $role->name) }}" name="name"
                                ...
                                @if ($permission->isNotEmpty())
                                    @foreach ($permission as $data)
                                        <div class="mt-3 flex items-center">
                                            <input {{($hasPermission->contains())}} type="checkbox" id="permission{{ daata->id }}"
                                                name="permission[]" value="{{ daata->id }}" class="rounded">
                                            <label for="permission{{ daata->id }}" class="ml-2">
                                                {{ daata->name }}
                                            </label>
                                        </div>
                                    @endforeach
